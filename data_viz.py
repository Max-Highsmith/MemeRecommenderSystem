...
ts_embeddedData  = tsne.fit_transform(nominalMemeData);
print("KL-Divergence");
...
estimator.fit(reducedMemeData);
print(estimator.means_);
color = estimator.predict(reducedMemeData);
print("BIC");
print(estimator.bic(reducedMemeData));
...
line = ax.scatter(xs, ys, zs, c=color,picker=5);
...
##initialization
##umOfGMMs = 1;
##eights = np.full([numOfGMMs],1/numOfGMMs);
